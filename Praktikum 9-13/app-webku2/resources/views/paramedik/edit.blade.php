    ...
    <form action="{{ route('paramedik.update', $paramedik->id) }}" method="post">
        ...
        <div class="form-group">
            ...
            <div class="input-group date" id="tgl_lahir" data-target-input="nearest">
                <input type="text" class="form-control datetimepicker-input" value="{{ $paramedik->tgl_lahir }}"
        ...
        <div class="form-group">
            ...
            <input type="tel" class="form-control" id="telpon" name="telpon" value="{{ $paramedik->telpon }}">
            @error('telpon')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
        <div class="form-group">
            ...
            <textarea name="alamat" id="alamat" class="form-control" rows="3">{{ $paramedik->alamat }}</textarea>
            @error('alamat')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
            ...
            <select name="kategori_id" id="kategori_id" class="custom-select">
                ...
                @foreach ($list_kategori as $kategori)
                    <option value="{{ $kategori->id }}"
                        {{ $paramedik->kategori_id == $kategori->id ? 'selected' : '' }}>
                        {{ $kategori->nama }}
                    </option>
                @endforeach
            </select>
            ...
            <select name="unit_kerja_id" id="unit_kerja_id" class="custom-select">
                ...
                @foreach ($list_unitkerja as $unitkerja)
                    <option value="{{ $unitkerja->id }}"
                        {{ $paramedik->unit_kerja_id == $unitkerja->id ? 'selected' : '' }}>
                        {{ $unitkerja->nama }}
                    </option>
                @endforeach
            </select>
