    ...
    <form action="{{ route('periksa.store') }}" method="post">
        ...
        <div class="form-group">
            ...
            <div class="input-group date" id="tanggal" data-target-input="nearest">
                <input type="text" class="form-control datetimepicker-input" value="{{ old('tanggal') }}"
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="tensi" name="tensi" value="{{ old('tensi') }}">
            @error('tensi')
                <small class="text-danger">{{ $message }}</small>
            @enderror
        </div>
            ...
            <select name="paramedik_id" id="paramedik_id" class="custom-select">
                ...
                @foreach ($list_paramedik as $paramedik)
                    <option value="{{ $paramedik->id }}"
                        {{ old('paramedik_id') == $paramedik->id ? 'selected' : '' }}>
                        {{ $paramedik->nama }}
                    </option>
                @endforeach
            </select>
